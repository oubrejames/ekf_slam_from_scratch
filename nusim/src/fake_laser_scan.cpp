...
public:
  ...
  : Node("fake_laser_scan"), count_(0)
  {
    ...
    noise_fake_laser_desc.description =
      "Variance for a zero mean noise applied to the fake laser scanner";
    ...
    real_obstacles_sub_ = this->create_subscription<visualization_msgs::msg::MarkerArray>(
      "/nusim/obstacles", 10,
      std::bind(&FakeLaser::real_obstacles_cb, this, std::placeholders::_1));
...
private:
  ...
  void do_a_laser_scan()
  {
    ...
    for (double i = angle_min; i < angle_max; i += angle_increment) {
      ...
      // add robot angle
      tf2::Quaternion q(laser_tf.transform.rotation.x, laser_tf.transform.rotation.y,
        laser_tf.transform.rotation.z, laser_tf.transform.rotation.w);
      ...
      for (size_t j = 0; j < obstacle_array.markers.size(); j++) {
        range = check_laser_intersect(
          {laser_tf.transform.translation.x,
            laser_tf.transform.translation.y}, {x_max_range, y_max_range}, {obstacle_array.markers.at(
              j).pose.position.x, obstacle_array.markers.at(j).pose.position.y}, 0.038);
  ...
  double check_wall_intersect(std::tuple<double, double> laser, std::tuple<double, double> max)
  {
    ...
    auto const d_left =
      std::sqrt(
      (x_left_arena - std::get<0>(laser)) * (x_left_arena - std::get<0>(laser)) +
      (y_left - std::get<1>(laser)) * (y_left - std::get<1>(laser)));
    auto const d_right =
      std::sqrt(
      (x_right_arena - std::get<0>(laser)) * (x_right_arena - std::get<0>(laser)) +
      (y_right - std::get<1>(laser)) * (y_right - std::get<1>(laser)));
    auto const d_top =
      std::sqrt(
      (x_top - std::get<0>(laser)) * (x_top - std::get<0>(laser)) +
      (y_top_arena - std::get<1>(laser)) * (y_top_arena - std::get<1>(laser)));
    auto const d_bot =
      std::sqrt(
      (x_bot - std::get<0>(laser)) * (x_bot - std::get<0>(laser)) +
      (y_bot_arena - std::get<1>(laser)) * (y_bot_arena - std::get<1>(laser)));
    ...
    while (flag) {
      ...
      if (((x_points.at(min_idx) - std::get<0>(laser)) / (std::get<0>(max) - std::get<0>(laser))) >
        0.0)
      {
        flag = false;
  ...
  double check_laser_intersect(
    std::tuple<double, double> laser, std::tuple<double, double> max,
    std::tuple<double, double> obstacle, double obs_radius)
  {
    // Calculate possible x and y componenets for both plus and minus
    auto const m = (std::get<1>(max) - std::get<1>(laser)) /
      (std::get<0>(max) - std::get<0>(laser));

    auto const a = 1 + m * m;

    auto const alpha = std::get<1>(laser) - m * std::get<0>(laser) - std::get<1>(obstacle);
    auto const b = 2 * (alpha * m - std::get<0>(obstacle));
    auto const c = std::get<0>(obstacle) * std::get<0>(obstacle) + alpha * alpha - obs_radius *
      obs_radius;
    ...
    if (disc > 0) {
      ...
      auto const dp = std::sqrt(
        (xp - std::get<0>(laser)) * (xp - std::get<0>(laser)) +
        (yp - std::get<1>(laser)) * (yp - std::get<1>(laser)));

      auto const dm = std::sqrt(
        (xm - std::get<0>(laser)) * (xm - std::get<0>(laser)) +
      ...
      if ((range < min_range) || (range > max_range) ||
        ((x - std::get<0>(laser)) / (std::get<0>(max) - std::get<0>(laser)) < 0.0) ||
        ( (y - std::get<1>(laser)) / (std::get<1>(max) - std::get<1>(laser)) < 0.0))
      {
        range = 0.0;
      }
    }
