...
public:
  ...
  : Node("basic_sensor"), count_(0)
  {
    ...
    real_obstacles_sub_ = this->create_subscription<visualization_msgs::msg::MarkerArray>(
      "/nusim/obstacles", 10, std::bind(
        &BasicSensor::real_obstacles_cb, this,
        std::placeholders::_1));

    // Publisher to publish sensed obstacles
    sensed_obstacles_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "fake_sensor", 10);
...
private:
  ...
  void timer_callback()
  {
    ...
    for (int i = 0; i < (int)sensed_obstacles.markers.size(); i++) {
      turtlelib::Transform2D Two{{sensed_obstacles.markers.at(i).pose.position.x,
        sensed_obstacles.markers.at(i).pose.position.y}};
      ...
      sensed_obstacles.markers.at(i).pose.position.x = Tro.translation().x +
        gaus_dist(get_random());
      sensed_obstacles.markers.at(i).pose.position.y = Tro.translation().y +
        gaus_dist(get_random());
